...
#define LOG_BA(ba) LOG("buffer_attr{fragsize=%d,maxlength=%d,minreq=%d,prebuf=%d,tlength=%d}", ba.fragsize, ba.maxlength, ba.minreq, ba.prebuf, ba.tlength);
  ...
  Stream::Init(Local<Object> target) {
    ...
    tpl->SetClassName(String::NewFromOneByte(isolate, (const uint8_t*)"PulseAudioStream", NewStringType::kNormal).ToLocalChecked());
    ...
    target->Set(String::NewFromOneByte(isolate, (const uint8_t*) "Stream", NewStringType::kNormal).ToLocalChecked(), cfn);
